...
// FILE: routes/registrationRequests.js
// Doctor, Pharmacy & Laboratory registration approval workflow API
// Routes mounted at: /api/registration-requests
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!role) {
        return res.status(400).json({ error: 'Missing Account Role (role). Please select Doctor, Pharmacy or Laboratory.' });
      }

      // ✅ FIXED: LABORATORY added to allowed roles
      if (!ALLOWED_ROLES.includes(role)) {
        console.warn(`⚠️ Registration Submit: Invalid role received: "${role}"`);
        return res.status(400).json({ error: `Invalid role: "${role}". Must be DOCTOR, PHARMACY or LABORATORY.` });
      }
...
router.get('/stats', verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    // ✅ FIXED: LABORATORY added to stats count
    const [
      pending,
      approvedToday,
      totalRejected,
      ...
      prisma.registrationRequest.count({ where: { role: 'DOCTOR' } }),
      prisma.registrationRequest.count({ where: { role: 'PHARMACY' } }),
      prisma.registrationRequest.count({ where: { role: 'LABORATORY' } }), // ✅ NEW
    ]);
...
/* ============================================================
   GET /api/registration-requests
   Admin only. Paginated list of all requests.
   Query: ?status=PENDING|APPROVED|REJECTED&role=DOCTOR|PHARMACY|LABORATORY&page=1&limit=10
   ============================================================ */
router.get('/', verifyToken, requireAdmin, async (req, res) => {
  try {
    const { status, role, page = '1', limit = '10' } = req.query;
    ...
    // ✅ FIXED: LABORATORY added to role filter
