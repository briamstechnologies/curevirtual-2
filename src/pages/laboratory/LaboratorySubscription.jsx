...
export default function LaboratorySubscription() {
  ...
  const userName = localStorage.getItem("userName") || localStorage.getItem("name") || "Laboratory Partner";
  ...
  const fetchPlansAndSubscription = async () => {
    try {
      ...
      const [plansRes, mySubRes] = await Promise.allSettled([
        api.get("/subscriptions/plans?module=laboratory"),
      ...
      if (plansRes.status === "fulfilled" && plansRes.value?.data?.plans) {
        ...
        setPlans([
          {
            id: "lab-monthly-default",
            module: "laboratory",
            name: "Lab Partner Plan (Monthly)",
            billingCycle: "monthly",
            priceGHS: 150,
            commissionRateOverride: 8,
            perks: ["8% Flat Commission Rate (saves 7%)", "Unlimited Monthly Order Volume", "Featured Search Placement", "24/7 Priority Partner Support"]
          },
          {
            id: "lab-annual-default",
            module: "laboratory",
            name: "Lab Partner Plan (Annual)",
            billingCycle: "annual",
            priceGHS: 1500,
            commissionRateOverride: 8,
            perks: ["Save GHS 300 vs Monthly Plan", "8% Flat Commission Rate (saves 7%)", "Unlimited Monthly Order Volume", "Featured Search Placement", "24/7 Priority Partner Support"]
          }
        ]);
      }
      ...
      if (mySubRes.status === "fulfilled" && mySubRes.value?.data?.subscriptions) {
        ...
        const labSubs = allSubs.filter((s) => s.plan?.module === "laboratory");
        const activeLabSub = labSubs.find((s) => s.status === "active");
        setMySubscription(activeLabSub || null);
        setSubHistory(labSubs);
      }
    } catch (err) {
      console.error("Error fetching lab subscription data:", err);
  ...
  const handleSubscribe = async (plan) => {
    ...
    if (mySubscription && mySubscription.status === "active") {
      toast.warn("You already have an active subscription for the laboratory module.");
      return;
    }
    ...
    if (paymentWindow) {
      paymentWindow.document.write(
        "<h3 style='font-family:sans-serif; text-align:center; margin-top:20%; color:#a855f7;'>Connecting to secure Paystack gateway... Please wait.</h3>"
      );
  ...
  return (
    <DashboardLayout role={role} user={{ name: userName }}>
      <div className="space-y-10 font-body min-h-screen text-[var(--text-main)] animate-in fade-in duration-700">
        ...
        {/* Banner Section */}
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-purple-950 via-slate-900 to-indigo-950 p-8 md:p-12 text-white shadow-2xl border border-purple-500/30">
          <div className="absolute -right-10 -bottom-10 opacity-10 pointer-events-none text-purple-400">
            <FaFlask size={320} />
          </div>
          
          <div className="relative z-10 max-w-3xl space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full bg-purple-500/20 text-purple-300 border border-purple-400/30 px-4 py-1.5 text-xs font-black uppercase tracking-widest backdrop-blur-md">
              <FaShieldAlt /> Lab Partner Monetization & Subscriptions
            </div>
            
            <h1 className="text-3xl md:text-5xl font-black tracking-tight leading-tight text-white">
              Upgrade to <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-200 to-emerald-300 underline decoration-amber-400">Lab Partner Plan</span>
            </h1>
            
            <p className="text-sm md:text-base text-slate-200 leading-relaxed font-medium">
              Reduce your platform commission from 15% down to a flat <strong className="text-amber-300 font-black">8% rate</strong>. Keep 92% of all your laboratory order revenue and enjoy priority placement.
            </p>
          </div>
        </section>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-lg font-black text-[var(--text-main)]">
                  {mySubscription.plan?.name || "Lab Partner Plan"}
                </h3>
        ...
        {/* Interactive Lab ROI Calculator Widget */}
        <div className="bg-[var(--bg-glass)] backdrop-blur-xl border border-[var(--border)] p-8 rounded-3xl shadow-xl space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-purple-500/15 text-purple-400 flex items-center justify-center text-lg">
              <FaChartLine />
            </div>
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-purple-400">
                Lab Financial ROI Calculator
              </span>
          ...
          <div className="grid md:grid-cols-12 gap-8 items-center">
            <div className="md:col-span-7 space-y-4">
              <div className="flex justify-between items-center text-sm font-bold">
                <span className="text-[var(--text-soft)]">Monthly Order Revenue:</span>
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(to right, #a855f7 0%, #a855f7 ${((monthlyOrderRevenue - 1000) / (30000 - 1000)) * 100}%, rgba(203, 213, 225, 0.3) ${((monthlyOrderRevenue - 1000) / (30000 - 1000)) * 100}%, rgba(203, 213, 225, 0.3) 100%)`
                }}
                className="w-full h-3 rounded-lg appearance-none cursor-pointer accent-purple-500 border border-[var(--border)] shadow-inner transition-all"
              />
              ...
              <div className="grid grid-cols-2 gap-4 text-xs pt-2">
                ...
                <div className="bg-[var(--bg-main)] p-3 rounded-xl border border-[var(--border)]">
                  <span className="text-[var(--text-muted)] block text-[10px] font-bold">Lab Partner 8% Cut</span>
            ...
            <div className="md:col-span-5 bg-gradient-to-br from-emerald-500/20 via-sky-500/10 to-transparent p-6 rounded-2xl border border-emerald-500/30 text-center space-y-2">
              ...
              <p className="text-[11px] text-[var(--text-soft)]">
                Save GHS {grossCommissionSaved.toFixed(0)}/mo in platform commission (- GHS 150 Lab Plan fee)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          ...
          <div className="bg-[var(--bg-glass)] border border-[var(--border)] rounded-3xl p-8 space-y-6 flex flex-col justify-between hover:border-purple-500/30 transition">
            <div className="space-y-4">
              ...
              <div className="space-y-3 pt-4 border-t border-[var(--border)] text-xs font-medium">
                <div className="flex items-center gap-2 text-[var(--text-soft)]">
                  <FaCheck className="text-purple-400" /> Tier 1 Commission: 15% (0–49 orders)
                </div>
                <div className="flex items-center gap-2 text-[var(--text-soft)]">
                  <FaCheck className="text-purple-400" /> Tier 2 Commission: 12% (50–199 orders)
                </div>
                <div className="flex items-center gap-2 text-[var(--text-soft)]">
                  <FaCheck className="text-purple-400" /> Tier 3 Commission: 10% (200+ orders)
                </div>
          ...
          {/* Card 2: Lab Partner Pro Plan */}
          <div className="relative bg-gradient-to-b from-purple-500/10 via-[var(--bg-glass)] to-[var(--bg-glass)] border-2 border-purple-500 rounded-3xl p-8 space-y-6 flex flex-col justify-between shadow-2xl">
            
            <div className="absolute top-0 right-0 bg-gradient-to-r from-purple-500 to-sky-500 text-white text-[10px] font-black uppercase tracking-widest px-6 py-1.5 rounded-bl-2xl shadow-md flex items-center gap-1.5">
            ...
            <div className="space-y-4">
              ...
              <h3 className="text-2xl font-black text-[var(--text-main)]">Lab Partner Plan</h3>
              ...
              <p className="text-xs text-[var(--text-soft)] leading-relaxed">
                Unlock <strong className="text-emerald-400">8% flat platform commission</strong> on every lab order regardless of volume.
              </p>
              ...
              <div className="space-y-3 pt-4 border-t border-[var(--border)] text-xs font-medium">
                ...
                <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
                  <FaCheckCircle className="text-purple-400" /> Unlimited Monthly Order Volume
                </div>
                <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
                  <FaCheckCircle className="text-purple-400" /> Featured Search Placement for Patients
                </div>
                <div className="flex items-center gap-2 text-[var(--text-main)] font-semibold">
                  <FaCheckCircle className="text-purple-400" /> Paystack Ghana MoMo & Card Gateway
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-2 ${
                mySubscription
                  ...
                  : "bg-gradient-to-r from-purple-500 to-sky-500 hover:opacity-95 text-white disabled:opacity-50"
              }`}
            >
              {mySubscription ? (
                <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-[var(--border)]/50 text-xs font-medium">
                  {subHistory.map((h, i) => (
                    <tr key={h.id || i}>
                      <td className="py-3 font-bold text-[var(--text-main)]">{h.plan?.name || "Lab Partner Plan"}</td>
                      <td className="py-3">
                      ...
                      <td className="py-3 text-[var(--text-soft)]">{h.startedAt ? new Date(h.startedAt).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' }) : "N/A"}</td>
        ...
        {paystackUrl && (
          <div className="fixed inset-0 bg-black/80 z-[9999] backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl w-full max-w-md p-6 md:p-8 shadow-2xl text-center space-y-6 animate-in zoom-in-95 duration-200">
              <div className="w-16 h-16 mx-auto rounded-full bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 text-2xl font-bold">
              ...
              <button 
                ...
                className="w-full py-3 bg-gradient-to-r from-purple-500 to-sky-500 hover:opacity-90 text-white rounded-2xl text-xs font-bold uppercase tracking-wider transition shadow-lg"
              >
