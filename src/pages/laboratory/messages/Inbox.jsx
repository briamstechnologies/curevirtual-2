...
import { FiSearch, FiSend, FiMail, FiUser, FiClock } from "react-icons/fi";
import { Link } from "react-router-dom";
import DashboardLayout from "../../../layouts/DashboardLayout";

export default function Inbox() {
  const [search, setSearch] = useState("");
  const userName = localStorage.getItem("userName") || "Laboratory"; // Layout ke liye zaroori
  ...
  return (
    // Yahan humne DashboardLayout wrap kar diya
          ...
          {filteredMessages.map((msg) => (
            <div
              ...
              className={`glass-panel rounded-3xl p-6 border transition-all hover:scale-[1.01] ${
                msg.unread ? "border-[var(--brand-blue)]" : "border-[var(--border)]"
              }`}
            >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      <h3 className="text-lg font-black text-[var(--text-main)]">{msg.subject}</h3>
                      {msg.unread && (
                    ...
                    <p className="text-sm text-[var(--text-soft)] mt-3">{msg.preview}</p>
                  </div>
                </div>
