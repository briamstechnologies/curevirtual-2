      ...
      <nav className="fixed w-full z-50 backdrop-blur-3xl border-b border-[var(--border)] px-4 md:px-6 py-3 bg-[var(--bg-glass)]/80 transition-all duration-300">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 md:p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white/80 hover:text-[var(--brand-orange)] transition-all shadow-sm min-h-[44px] min-w-[44px] flex items-center justify-center active:scale-95"
            ...
            <button
              ...
              className="hidden md:block px-4 md:px-5 py-2 font-black text-xs uppercase tracking-widest text-white/80 hover:text-white transition-colors"
            >
              Sign In
            </button>
            <button
      ...
      <section className="relative min-h-[100dvh] flex items-center pt-28 pb-10 md:pt-24 md:pb-0 overflow-hidden">
        ...
        <div className="page-container grid lg:grid-cols-2 gap-10 md:gap-12 items-center">
          <div className="text-center lg:text-left space-y-6 md:space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-[var(--brand-orange)] text-[10px] font-black uppercase tracking-[0.2em] animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-4 md:space-y-5">
              ...
              />
              <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-black text-white leading-[0.9] tracking-tighter uppercase">
            ...
            <p className="text-lg md:text-xl text-white/90 max-w-xl leading-relaxed font-bold mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4 md:pt-6">
              <button
                onClick={() => navigate("/register")}
                className="btn btn-primary !py-4 md:!py-5 !px-8 md:!px-12 text-sm shadow-[0_20px_40px_-10px_rgba(34,197,94,0.3)] w-full sm:w-auto hover:scale-105 active:scale-95 transition-all"
              >
                Start Consultation <FaArrowRight />
              </button>
              <button
                onClick={() => navigate("/login")}
                className="btn btn-glass !py-4 md:!py-5 !px-8 md:!px-12 text-sm text-white w-full sm:w-auto border-2 border-white/30 hover:border-white hover:bg-white/10 transition-all"
              >
                Login to Portal
              </button>
            </div>

            <div className="flex items-center justify-center lg:justify-start gap-8 md:gap-12 pt-8 md:pt-10">
              <div className="text-center">
                <p className="text-3xl md:text-4xl font-black text-white">24/7</p>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/60">Support</p>
              </div>
              <div className="w-px h-10 bg-white/20"></div>
              <div className="text-center">
                <p className="text-3xl md:text-4xl font-black text-white">Privacy</p>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/60">Protected</p>
              </div>
            </div>
          </div>
      ...
      <section id="features" className="py-20 md:py-24 relative bg-[var(--bg-main)]">
        <div className="page-container">
          <div className="text-center mb-12 md:mb-20 space-y-4 md:space-y-6">
            <h2 className="text-xs font-black text-white uppercase tracking-[0.4em] opacity-60 animate-pulse">
              Our Capabilities
            </h2>
            <h3 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none text-white">
      ...
      >
        <div className="page-container flex flex-col lg:flex-row items-center gap-16 md:gap-24">
          <div className="lg:w-1/2 space-y-8 md:space-y-10 text-center lg:text-left">
            <h2 className="text-white/60 font-black text-xs uppercase tracking-[0.3em]">
              The Ecosystem
            </h2>
            <h3 className="text-4xl md:text-5xl lg:text-6xl font-black leading-tight tracking-tighter text-white">
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-8">
              <div className="p-6 md:p-8 bg-white/5 backdrop-blur-md !rounded-[2rem] border border-white/10 hover:-translate-y-2 transition-transform duration-500">
              ...
              <div className="p-6 md:p-8 bg-white/5 backdrop-blur-md !rounded-[2rem] border border-white/10 hover:-translate-y-2 transition-transform duration-500 delay-100">
      ...
      <footer id="contact" className="py-16 md:py-20">
        <div className="page-container flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12 border-t border-[var(--border)] pt-12 md:pt-16">
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-white/40 md:ml-4">
              &copy; 2025 ALL SYSTEMS NOMINAL
            </p>
          </div>

          <div className="flex flex-wrap justify-center gap-8 md:gap-12 text-xs font-black uppercase tracking-widest text-white/60">
...
function NavLink({ href, children }) {
  return (
    <a
      href={href}
      className="text-[11px] font-black uppercase tracking-[0.2em] text-white/70 hover:text-white active:scale-95 transition-all"
    >
      {children}
    </a>
  );
...
function FeatureCard({ icon, title, text, color }) {
  ...
  return (
    <div
      className={`card group p-6 md:p-10 hover:-translate-y-2 transition-all cursor-default border-white/10 bg-white/5 backdrop-blur-lg ${dynamicColor} h-full flex flex-col`}
    >
      <div
        className={`h-16 w-16 rounded-[1.5rem] bg-white/10 border border-white/20 flex items-center justify-center text-3xl mb-8 group-hover:scale-110 group-hover:rotate-6 transition-all shadow-inner`}
      >
        {icon}
      </div>
