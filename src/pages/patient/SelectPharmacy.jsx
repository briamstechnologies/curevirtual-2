...
export default function PatientSelectPharmacy() {
  ...
  const [list, setList] = useState(() => {
    const cached = localStorage.getItem("cached_pharmacy_list");
  ...
  const load = useCallback(async () => {
    try {
      ...
      setList(parsedItems);
      // only cache the default view (no search parameters applied)
      if (!q && !lat && !lng) {
        localStorage.setItem("cached_pharmacy_list", JSON.stringify(parsedItems));
      }
    } catch {
      setList([]);
    } finally {
