...
export default function PatientProfile() {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!userId) {
      toast.error("User identity missing. Please log in again.");
      return;
    }
    try {
      setSaving(true);
      ...
      updateUser({
        ...
        // Add other fields if needed in context
      });
            ...
            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              {form.referenceId && (
                <div className="md:col-span-2 space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-green)] ml-1">
                    Patient Reference ID
                  </label>
                  <input
                    type="text"
                    className="w-full bg-[var(--bg-main)] border border-[var(--brand-green)]/30 rounded-2xl py-3.5 px-4 text-xs font-bold text-[var(--brand-green)] outline-none cursor-not-allowed"
