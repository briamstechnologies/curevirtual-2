...
export default function MyLaboratory() {
  const role = 'PATIENT';
  ...
  const load = async () => {
    try {
      const res = await api.get('/laboratory/patient/selected', {
        params: { patientId },
      });
  ...
  const removeMap = async (mapId) => {
    try {
      await api.delete(`/laboratory/patient/select/${mapId}`);
      toast.success('Laboratory unlinked.');
      await load();
    } catch (err) {
      toast.error('Failed to unlink laboratory.');
    }
  };

  const togglePreferred = async (mapId, next) => {
    try {
      await api.patch(`/laboratory/patient/select/${mapId}/preferred`, {
        preferred: next,
      });
      toast.success(next ? 'Prioritized' : 'Deprioritized');
      await load();
    } catch (err) {
      toast.error('Failed to update laboratory priority.');
    }
  };
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {items.length === 0 ? (
                  <tr>
                    ...
                    >
                      No laboratories linked to your identity.
                    </td>
                  </tr>
                ) : (
                  items.map((r) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 text-xs font-bold text-[var(--text-soft)]">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[var(--text-main)]">
                    {view.labProfile?.services || "General Diagnostics"}
                  </p>
                </div>
              </div>
            </div>
