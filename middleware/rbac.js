 ...
 * Role-Based Access Control (RBAC) Middleware
 *
 * Hierarchy: SUPERADMIN > ADMIN > DOCTOR/PHYSICIAN_ASSISTANT > PATIENT/PHARMACY/SUPPORT
 *
 * Usage:
 * router.get("/superadmin/users", requireRole("SUPERADMIN"), handler)
 * router.get("/admin/reports", requireRole(["SUPERADMIN", "ADMIN"]), handler)
 ...
 */
const requireHierarchy = (minRole) => {
  // Added PHYSICIAN_ASSISTANT at level 3, matching DOCTOR level
  const hierarchy = {
    SUPERADMIN: 5,
    ADMIN: 4,
    DOCTOR: 3,
...
const permissions = {
  ...
  // Added PHYSICIAN_ASSISTANT Matrix - Inherits exact doctor permissions
  PHYSICIAN_ASSISTANT: {
    viewOwnPatients: true,
    createPrescriptions: true,
    editOwnPrescriptions: true,
